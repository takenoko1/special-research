    ...
    # メインループ #########################################################
        try:
            while True:
                ...
                # 体 描画
                mp_drawing.draw_landmarks(
                    self.frame, holistic_results.pose_landmarks, mp_holistic.POSE_CONNECTIONS)
                ...
                #""" 体中心描画
                if holistic_results.pose_landmarks.landmark is not None: # 一部が映った時点で全体が推測される
                    shoulder_L = holistic_results.pose_landmarks.landmark[mp_holistic.PoseLandmark.LEFT_SHOULDER]
                    shoulder_R = holistic_results.pose_landmarks.landmark[mp_holistic.PoseLandmark.RIGHT_SHOULDER]
                    center_x = (shoulder_L.x*self.width + shoulder_R.x*self.width) / 2
                    center_y = (shoulder_L.y*self.height + shoulder_R.y*self.height) / 2
                self.frame = cv2.circle(self.frame, (int(center_x), int(center_y)), 10, (0, 0, 255), thickness=-1)
                #"""
